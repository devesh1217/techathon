...
export default function PushNotificationManager() {
    ...
    if (!isSupported) {
        return <p>Push notifications are not supported in this system.</p>;
    }
    ...
    return (
        <div className='flex flex-col text-center justify-center items-center gap-5 p-5 w-full border-t-2 border-t-white bg-white text-black'>
            <h3 className='text-4xl'>Enable Real Time Hzarda Notification</h3>
            {(subscription && localStorage.getItem('push-id')) ? (
                <div className='flex flex-col justify-center items-center'>
                    <p>You are required grant to Push Notifications permission fo real time smart hazard notifications.</p>
